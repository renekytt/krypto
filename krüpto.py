def krüpteeri(sisend,võti):
    tähestik=["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z","ö","ä","ü","õ"," "]
    võti_õige_pikkusega=pikkused_samaks(sisend,võti)
    sisend=sisend.lower()
    krüpteeritud=""
    for i in range(len(sisend)):
        krüpteeritud+=tähestik[(tähestik.index(võti_õige_pikkusega[i])+ tähestik.index(sisend[i]))%31]
    return krüpteeritud
...
def dekrüpteeri(krüpteeritud,võti):
    ...
    for i in range(len(krüpteeritud)):
        dekrüpteeritud+=tähestik[(tähestik.index(krüpteeritud[i])-tähestik.index(võti_õige_pikkusega[i]))%31]
